-- ============================================================
-- Company 2 — Complete Seed Data
-- Run AFTER schema.sql: mysql -u root company2_db < seeds.sql
-- ============================================================
...
-- Manager gets all EXCEPT permissions management + settings delete
INSERT INTO `role_permissions` (`role_id`, `permission_id`)
SELECT 3, `id` FROM `permissions`
WHERE `slug` NOT IN ('permissions.create','permissions.edit','permissions.delete','settings.delete');

-- Staff gets view + edit on bookings, reviews; view only on others
INSERT INTO `role_permissions` (`role_id`, `permission_id`)
...
(1, 1, 'Super Admin', 'admin@company2.sa',
 '$2y$12$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', -- password: password (replace in production)
 1, 'active', NOW());
...
-- ─────────────────────────────────────────
-- NOTIFICATION SETTINGS (default for admin)
-- ─────────────────────────────────────────
...
-- SETTINGS
-- ─────────────────────────────────────────
INSERT INTO `settings` (`tenant_id`, `key`, `value`, `type`, `group`) VALUES
